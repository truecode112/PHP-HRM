...
class Employees extends BaseController {
	...
	public function employees_list() {
		...
		$data = array();
          ...
          foreach($staff as $r) {		
			...
			if(isset($employee_detail['designation_id'])){
				$idesignations = $DesignationModel->where('designation_id',$employee_detail['designation_id'])->first();
				$designation_name = $idesignations['designation_name'] ?? 'NA';
			} else {
				$designation_name = '--';
			}
	...
	public function add_employee() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			}elseif($this->request->getPost('department_id',FILTER_SANITIZE_STRING) == 'add_new'){
				$Return['error'] = lang('Employees.xin_employee_error_department');
			}elseif($this->request->getPost('designation_id',FILTER_SANITIZE_STRING) == 'add_designation'){
				$Return['error'] = lang('Employees.xin_employee_error_designation');
			}elseif($this->request->getPost('office_shift_id',FILTER_SANITIZE_STRING) == 'add_new_shift'){
				$Return['error'] = lang('Employees.xin_office_shift_field_error');
			}elseif($this->request->getPost('role',FILTER_SANITIZE_STRING) == 'add_new_role'){
				$Return['error'] = lang('Employees.xin_employee_error_staff_role');
			}elseif($validation->hasError('hourly_rate')){
				$Return['error'] = $validation->getError('hourly_rate');
			}elseif($validation->hasError('payment_mode')){
				$Return['error'] = $validation->getError('payment_mode');
			}
			...
			$payment_mode = $this->request->getPost('payment_mode',FILTER_SANITIZE_STRING);
			...
			if ($result == TRUE) {
				...
				$Return['redirect_url'] = site_url('erp/employee-details').'/'.uencode($user_id);
	...
	public function update_basic_info() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$employee_id = $this->request->getPost('employee_id',FILTER_SANITIZE_STRING);
			$allergies = $this->request->getPost('allergies', FILTER_SANITIZE_STRING);
	...
	public function update_contract_info() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			}elseif($validation->hasError('contract_type')){
				$Return['error'] = $validation->getError('contract_type');
			}
			elseif($validation->hasError('department_id')){
				$Return['error'] = $validation->getError('department_id');
			} elseif($validation->hasError('designation_id')){
			...
			}elseif (!empty($date_of_leaving) && strtotime($date_of_leaving) <= strtotime($date_of_joining)) {
				$Return['error'] = lang('Main.custom_date_greater_than');
			}elseif($validation->hasError('payment_mode')){
				$Return['error'] = $validation->getError('payment_mode');
			}
			if($Return['error']!=''){
			...
			//staff details
			$date_of_joining = $this->request->getPost('contract_date',FILTER_SANITIZE_STRING);
			$contract_type = $this->request->getPost('contract_type',FILTER_SANITIZE_STRING);
			$department_id = $this->request->getPost('department_id',FILTER_SANITIZE_STRING);
			$designation_id = $this->request->getPost('designation_id',FILTER_SANITIZE_STRING);
			$basic_salary = $this->request->getPost('basic_salary',FILTER_SANITIZE_STRING);
			$hourly_rate = $this->request->getPost('hourly_rate',FILTER_SANITIZE_STRING);
	...
	public function dialog_user_data()
	{
		...
		$salary = $request->getGet('emp_salary');
		$data = [
				'field_id' => $id,
