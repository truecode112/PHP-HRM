...
class Payroll extends BaseController
{
	...
	public function payslip_list()
	{
		...
		$staff_unique_id = $this->request->getVar('all_staffs', FILTER_SANITIZE_STRING);
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		...
		$staffQuery = $UsersModel->where('company_id', $company_id)
			->where('user_type', 'staff');

		if ($employee_id) {
			$staffQuery->where('user_id', $employee_id);
		}

		if ($department_id) {
			$userids = $StaffdetailsModel->select("user_id")->where("department_id", $department_id)->findAll();
			if ($userids) {
				$userids = array_column($userids, "user_id");
				$staffQuery->whereIn('user_id', $userids);
			}
		}

		if ($staff_unique_id) {
			$staffQuery->where('user_id', $staff_unique_id);
		}
		...
		foreach ($staff as $r) {
			...
			$ibasic_salary = $user_detail['basic_salary'] ?? 0;
	...
	public function advance_salary_list()
	{
		...
		foreach ($get_data as $r) {
			...
			$d2 = explode('-', $r['end_date']);
			$get_month2 = date('F', mktime(0, 0, 0, $d[1], 10));
			$month_year = $get_month . ', ' . $d[0] . ' to ' . $get_month2 . ', ' . $d2[0];
			// user info
	...
	public function payslip_history_list()
	{
		...
		foreach ($payslip as $r) {
			...
			$salary_month = ($r['salary_month'] == 'undefined' ? $r['year_to_date'] : $r['salary_month']);
			$smonth = strtotime($salary_month);
			$smonth = date('F, Y', $smonth);
			$salary_month = set_date_format($salary_month);
			$net_salary = '<h6 class="text-success">' . number_to_currency($inet_salary, $xin_system['default_currency'], null, 2) . '</h6>';
			$combhr = $view;
			$links = '
					' . $uname . '
	...
	public function add_advance_salary()
	{
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			} else if (strtotime($end_date) <= strtotime($month_year)) {
				$Return['error'] = lang('Main.custom_date_greater_than_payroll');
				$this->output($Return);
			} else {
				$month_year = $this->request->getPost('month_year', FILTER_SANITIZE_STRING);
