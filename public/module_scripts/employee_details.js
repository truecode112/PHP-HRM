$(document).ready(function () {
	...
	$('.view-modal-data').on('show.bs.modal', function (event) {
		...
		$.ajax({
			...
			type: "GET",
			data: 'jd=1' + field_add + 'field_id=' + field_id + '&uid=' + $('#user_id').val() + '&emp_salary=' + $('#employee_basic_salary').val(),
...
$("#employee_add_allowance_type").on("change", function () {
	var sel_val = $(this).val();
	if (sel_val == 2) {
		$('#percentage_window').removeClass('d-none');
		$('#amount_to_be_calculate').attr('readonly', true);
	} else {
		$('#percentage_window').addClass('d-none');
		$('#amount_to_be_calculate').removeAttr('readonly', true);

	}
})

$('#employee_percentage').on("keyup", function () {
	let percentage = $(this).val();
	let salary = $('#employee_basic_salary').val();
	var result = salary * (percentage / 100);
	$('#amount_to_be_calculate').val('')
	$('#amount_to_be_calculate').val(result);
})


const editEmployeeAllowance = (select) => {
	try {
		var sel_val = select.value;
		if (sel_val == 2) {
			$('#edit_percentage_window').removeClass('d-none');
			$('#edit_amount_to_be_calculate').attr('readonly', true);
		} else {
			$('#edit_percentage_window').addClass('d-none');
			$('#edit_amount_to_be_calculate').removeAttr('readonly', true);
...
const calculateEmployeeAllowancePercentage = (input) => {
	try {
		let percentage = input.value
		let salary = $('#employee_basic_salary').val();
		var result = salary * (percentage / 100);
		$('#edit_amount_to_be_calculate').val('')
		$('#edit_amount_to_be_calculate').val(result);
	} catch (error) {
		console.log(error);
	}
}
...
$("#allownace_type").on("change", function () {
	var sel_val = $(this).val();
	if (sel_val == 2) {
		$('#percentage_window_allowance').removeClass('d-none');
		$('#allowance_amount_to_be_calculate').attr('readonly', true);
	} else {
		$('#percentage_window_allowance').addClass('d-none');
		$('#allowance_amount_to_be_calculate').removeAttr('readonly', true);
	}
})

$('#allowance_employee_percentage').on("keyup", function () {
	let percentage = $(this).val();
	let salary = $('#employee_basic_salary').val();
	var result = salary * (percentage / 100);
	$('#allowance_amount_to_be_calculate').val('')
	$('#allowance_amount_to_be_calculate').val(result);
})


const calculateEmployeeCommissionPercentage = (input) => {
	try {
		let percentage = input.value
		let salary = $('#employee_basic_salary').val();
		var result = salary * (percentage / 100);
		$('#edit_commission_amount_to_be_calculate').val('')
		$('#edit_commission_amount_to_be_calculate').val(result);
	} catch (error) {
		console.log(error);
	}
}

const editEmployeeCommission = (select) => {
	try {
		var sel_val = select.value;
		if (sel_val == 2) {
			$('#edit_commission_percentage_window').removeClass('d-none');
			$('#edit_commission_amount_to_be_calculate').attr('readonly', true);
		} else {
			$('#edit_commission_percentage_window').addClass('d-none');
			$('#edit_commission_amount_to_be_calculate').removeAttr('readonly', true);
		}
	} catch (error) {
		console.log(error);
	}
...
$("#statutory_add").on("change", function () {
	var sel_val = $(this).val();
	if (sel_val == 2) {
		$('#statutory_window_allowance').removeClass('d-none');
		$('#statutory_amount_to_be_calculate').attr('readonly', true);
	} else {
		$('#statutory_window_allowance').addClass('d-none');
		$('#statutory_amount_to_be_calculate').removeAttr('readonly', true);
	}
})

$('#statutory_employee_percentage').on("keyup", function () {
	let percentage = $(this).val();
	let salary = $('#employee_basic_salary').val();
	var result = salary * (percentage / 100);
	$('#statutory_amount_to_be_calculate').val('')
	$('#statutory_amount_to_be_calculate').val(result);
})

const calculateEmployeeStatutoryPercentage = (input) => {
	try {
		let percentage = input.value
		let salary = $('#employee_basic_salary').val();
		var result = salary * (percentage / 100);
		$('#edit_statutory_amount_to_be_calculate').val('')
		$('#edit_statutory_amount_to_be_calculate').val(result);
	} catch (error) {
		console.log(error);
	}
}

const changeEmployeeStatutoryPercentage = (select) => {
	try {
		var sel_val = select.value;
		if (sel_val == 2) {
			$('#edit_statutory_percentage_window').removeClass('d-none');
			$('#edit_statutory_amount_to_be_calculate').attr('readonly', true);
		} else {
			$('#edit_statutory_percentage_window').addClass('d-none');
			$('#edit_statutory_amount_to_be_calculate').removeAttr('readonly', true);
		}
	} catch (error) {
		console.log(error);
	}
...
$("#other_payment_add").on("change", function () {
	var sel_val = $(this).val();
	if (sel_val == 2) {
		$('#other_payment_window_allowance').removeClass('d-none');
		$('#other_payment_amount_to_be_calculate').attr('readonly', true);
	} else {
		$('#other_payment_window_allowance').addClass('d-none');
		$('#other_payment_amount_to_be_calculate').removeAttr('readonly', true);
	}
})

$('#other_payment_employee_percentage').on("keyup", function () {
	let percentage = $(this).val();
	let salary = $('#employee_basic_salary').val();
	var result = salary * (percentage / 100);
	$('#other_payment_amount_to_be_calculate').val('')
	$('#other_payment_amount_to_be_calculate').val(result);
})

const calculateEmployeeOtherPaymentPercentage = (input) => {
	try {
		let percentage = input.value
		let salary = $('#employee_basic_salary').val();
		var result = salary * (percentage / 100);
		$('#edit_other_payment_amount_to_be_calculate').val('')
		$('#edit_other_payment_amount_to_be_calculate').val(result);
	} catch (error) {
		console.log(error);
	}
}

const changeEmployeeOtherPaymentPercentage = (select) => {
	try {
		var sel_val = select.value;
		if (sel_val == 2) {
			$('#edit_other_payment_percentage_window').removeClass('d-none');
			$('#edit_other_payment_amount_to_be_calculate').attr('readonly', true);
		} else {
			$('#edit_other_payment_percentage_window').addClass('d-none');
			$('#edit_other_payment_amount_to_be_calculate').removeAttr('readonly', true);
		}
	} catch (error) {
		console.log(error);
	}
